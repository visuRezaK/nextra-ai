...
Supabase tables used: `contacts` (leads from both booking form and chatbot), `chat_sessions`, `chat_messages`, `chat_memory`, `kb_documents` (pgvector, 1536-dim cosine), plus `profiles` (staff roles), `model_config` / `prompt_versions` (runtime config), `eval_questions` / `eval_runs` / `eval_results` (evaluation), and `lead_notes` (CRM timeline).
...
The panel is **bilingual**: every Persian label carries its English term beside it — use the `<En>` component and `PageTitle`'s `en` prop (`components/admin/ui.tsx`), or `leadStatusLabel()` («جدید (New)») where JSX can't go, such as `<option>` text and table cells. Keep new admin UI that way.

**Admin numbers render in Latin digits, the public site's in Persian.** The `fa()` / `faPct()` / `faDate()` / `faCad()` helpers use the `-u-nu-latn` locale extension (`fa-IR-u-nu-latn`, `fa-IR-u-ca-gregory-nu-latn`) — fa-IR grouping and RTL marks, Latin numerals, because the owner reads the panel next to Supabase and spreadsheets. Despite the `fa` prefix, these do **not** emit Persian digits. Percentages go through `faPct()` → «38%»: Latin sign, **after** the number (the Persian «٪۳۸» form leads with the sign, which reads wrong next to Latin digits). A few admin client components (`evaluation/eval-client.tsx`, `playground/playground-client.tsx`, `knowledge/upload-form.tsx`, `knowledge/test-search.tsx`) redeclare a local `fa` and inline their own `%` — keep them in sync. Do **not** touch `app/[locale]/**` or the chatbot's Persian copy (`lib/chatbot/prompts.ts`, the `SEED_QUESTIONS` in `evaluation/actions.ts`), which stay Persian for Persian readers.

**Leads = the CRM.** There is no separate CRM tab: `/admin/leads` (labelled «CRM — لیدها») is the pipeline list and `/admin/leads/[id]` is where a lead is worked (stage, next follow-up, amount, notes). `/admin/reports` reads the same table for the funnel, pipeline value by stage, monthly revenue and lead-source ROI. Money is **Canadian dollars** (`contacts.amount_cad numeric(12,2)`, rendered by `faCad()` as «CAD ۵٬۰۰۰٫۵۰» — a bare `$` would read as USD). PostgREST can return `numeric` as a **string**, so `Number()` it before summing. The stage vocabulary — `new | contacted | qualified | proposal | won | lost`, their Persian labels, badge tones, and the `isOverdue()` predicate — lives in `lib/admin/leads.ts` and is shared by the list, the detail page, `app/admin/leads/actions.ts`, the CSV export and the dashboard; **keep `LEAD_STATUSES` in sync with the `contacts_status_check` constraint in `admin6.sql`**. Reads gate on `requireRole(["operator", "viewer"])`, mutations on `requireRole(["operator"])`. `contacts.status` defaults to `'new'` in SQL, so the four capture paths (booking form, chatbot `captureLead`, voice, Telegram) insert unchanged. Status changes auto-append a `kind='status'` row to `lead_notes`, which is what makes the activity timeline a single query.

### Database migrations
Schema lives in `supabase/*.sql` (`schema.sql`, `chatbot.sql`, `admin.sql`, `admin2`–`admin7.sql`), applied **by hand in the Supabase SQL editor** — there is no migration tool. Files are idempotent and each admin phase adds one; a new table or feature does nothing (its pages/actions fail soft) until the corresponding SQL is run.

The fail-soft convention is load-bearing and worth copying: a page selects the new columns first, and if `data` comes back `null` (PostgREST errors with «column … does not exist»), it re-runs a query with only the old columns and hides the new UI. A valid empty result is `[]`, which is truthy — `null` really does mean "the SQL hasn't been run". See `app/admin/conversations/page.tsx`.

**When two migrations touch the same table, each must degrade on its own.** `contacts` is the live example: admin6 adds the stage, admin7 adds the money, so the leads pages step down a tier at a time (`money` → `crm` → `base` in `TIER_COLUMNS`, see `app/admin/leads/page.tsx`). Folding both into one column list would make an admin6-only database lose the working pipeline form. For *writes*, the equivalent is catching `PGRST204`/`42703` and retrying without the new columns — see `updateLeadAction` in `app/admin/leads/actions.ts` and `appendMessages` in `lib/chatbot/memory.ts`.

**Gotcha:** the PostgREST type parser can't infer a row shape from a column list chosen at runtime (a ternary or a `string` param inside `.select()`) — rows type as `ParserError`/`GenericStringError` and every property access fails to compile. Declare the row type explicitly and cast (`LeadRow`, `ExportRow`, `LeadDetailRow`). A literal string argument parses fine.
